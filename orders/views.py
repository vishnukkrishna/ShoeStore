...
def checkOut(request):
    ...
    try:
        ...
        payment = client.order.create(
            {
                "amount": int(cart.get_grand_total()) * 100,
                "currency": "INR",
                "payment_capture": 1,
            }
        )
...
def orderSuccessfully(request):
    ...
    for item in order_items:
        ...
        ordered_item = OrderItem.objects.create(
            ...
            item_price=item.get_product_price(),
            quantity=item.quantity,
            item_total=item.get_sub_total(),
        )
    ...
    return render(
        request, "orders/orderSuccessfully.html", {"order_id": razorpay_order_id}
    )
...
def order_tracking(request, item_id):
    ...
    context = {"item": item, "current_date": current_date}
...
def sales(request):
    ...
    if request.method == "POST":
        ...
        order_items = OrderItem.objects.filter(
            order__ordered_date__gte=start_date, order__ordered_date__lte=end_date
        )
