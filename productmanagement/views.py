...
def brandManagement(request):
    dict_brand = {"brand": Brand.objects.all().order_by("id")}
...
def addProducts(request):
    ...
    if request.method == "POST":
        ...
        for image in multi_image:
            multipleImage.objects.create(product=add_product, images=image)
...
def addVariant(request):
    if request.method == "POST":
        ...
        variant = Variation.objects.create(
            color=color_intance, product=product_instance
        )
