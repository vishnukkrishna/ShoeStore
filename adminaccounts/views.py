...
def index(request):
    ...
    payments = Payment.objects.filter(
        paid_date__gte=four_days_ago, paid_date__lte=today
    )

    # Getting the sales amount per day
    sales_by_day = (
        payments.annotate(day=TruncDay("paid_date"))
        .values("day")
    ...
    sales_dates = (
        Payment.objects.annotate(sale_date=Cast("paid_date", output_field=DateField()))
        .values("sale_date")
        .distinct()
    )
...
def bannerList(request):
    context = {"banner": Carousel_Home.objects.all().order_by("id")}
...
def addBanner(request):
    if request.method == "POST":
        ...
        addbanner = Carousel_Home.objects.create(
            carousel_heading=carousel_heading,
...
def editBanner(request, id):
    ...
    if request.method == "POST":
        ...
        if Carousel_Home.objects.filter(carousel_heading=carousel_heading).exists():
            ...
            edit_banner.update(
                carousel_heading=carousel_heading, carousel_text=carousel_text
            )
